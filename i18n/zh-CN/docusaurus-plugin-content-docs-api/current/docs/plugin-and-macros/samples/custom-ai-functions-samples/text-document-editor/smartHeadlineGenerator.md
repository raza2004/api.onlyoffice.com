...
该函数从当前选中的文本（段落、幻灯片或章节）生成清晰且相关的标题。该标题旨在准确表达内容，而非刻意追求吸引眼球。
...
| 名称  | 类型   | 示例          | 描述                                                                                      |
|-------|--------|---------------|-------------------------------------------------------------------------------------------|
| scope | string | "selection"   | 从 'currentParagraph'（当前段落）、'selection'（选中内容）或 'entireDocument'（整个文档）中提取内容（默认为 'selection'）。 |
