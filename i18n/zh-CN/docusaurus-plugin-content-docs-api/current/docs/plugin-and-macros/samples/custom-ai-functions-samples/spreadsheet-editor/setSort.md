...
该函数对给定范围（或活动/选中范围）按最多三列进行排序，支持使用单元格引用、列索引、命名范围或列标题。支持升序/降序排列以及可选的标题行检测。

## 提示词

- 将单元格 A1:D10 按第一列升序排序
- 按 Column1 升序对表格排序
- 按第二列降序排序
- 将 A1:D10 按 Column1 升序、再按 Column2 降序排序
- 按第一列对选中范围排序
...
| 名称       | 类型                         | 示例           | 描述                                                                                                               |
| ---------- | ---------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------ |
| range      | string                       | "A1:D10"       | 要排序的单元格范围。若省略则使用活动或选中范围。                                                                   |
| key1       | string \| ApiRange \| number | "A1"           | 第一个排序字段，可指定为范围、命名范围引用或范围内的列索引（从 1 开始）。                                          |
| key1Name   | string                       | "Name"         | 第一个排序字段的列名或标题，将自动解析为对应的列编号。                                                             |
| sortOrder1 | string                       | "xlAscending"  | 第一个排序字段的排序方式，可为 "xlAscending" 或 "xlDescending"。默认值为 "xlAscending"。                           |
| key2       | string \| ApiRange \| number | "C3"           | 第二个排序字段，可指定为范围、命名范围引用或范围内的列索引（从 1 开始）。                                          |
| key2Name   | string                       | "Age"          | 第二个排序字段的列名或标题，将自动解析为对应的列编号。                                                             |
| sortOrder2 | string                       | "xlDescending" | 第二个排序字段的排序方式，可为 "xlAscending" 或 "xlDescending"。默认值为 "xlAscending"。                           |
| key3       | string \| ApiRange \| number | "D5"           | 第三个排序字段，可指定为范围、命名范围引用或范围内的列索引（从 1 开始）。                                          |
| key3Name   | string                       | "Country"      | 第三个排序字段的列名或标题，将自动解析为对应的列编号。                                                             |
| sortOrder3 | string                       | "xlDescending" | 第三个排序字段的排序方式，可为 "xlAscending" 或 "xlDescending"。默认值为 "xlAscending"。                           |
