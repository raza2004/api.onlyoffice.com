...
该函数对选中单元格或指定范围执行基本数学运算（加、减、乘、除、中位数、平均值），并将结果写入指定单元格或该列最后已使用单元格的下方。

## 提示词

- 计算选中单元格的总和
- 计算选中范围的总和并写入单元格 A12
- 将单元格 A5 从单元格 B7 中减去，结果写入 C10
- 将单元格 C3 除以单元格 D4
- 将选中单元格相乘
- 求选中单元格的中位数
...
| 名称       | 类型   | 示例     | 描述                                                                                                                |
| ---------- | ------ | -------- | ------------------------------------------------------------------------------------------------------------------- |
| range      | string | "A1:A10" | 包含待计算值的单元格范围。用于加、乘、中位数、平均值运算。若省略则使用当前选中区域。 |
| targetCell | string | "A12"    | 写入结果的单元格。若省略则写入 A 列最后已使用单元格的下方。                         |
| operation  | string | "add"    | 要执行的数学运算：'add'、'subtract'、'multiply'、'divide'、'median'、'average'。默认为 'add'。 |
