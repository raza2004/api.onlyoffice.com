...
该函数收集所有表单字段（键/占位符），将字段名重命名为 UPPER_SNAKE_CASE 格式，并可选择更新占位符。
...
| 名称   | 类型   | 示例                   | 描述                     |
| ------ | ------ | ---------------------- | ------------------------ |
| prompt | string | "Rename all form keys" | 给 AI 代理的指令         |
