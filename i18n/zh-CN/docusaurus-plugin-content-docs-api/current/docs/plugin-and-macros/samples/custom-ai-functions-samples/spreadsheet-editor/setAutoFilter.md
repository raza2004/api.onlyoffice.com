...
该函数对给定单元格范围或当前活动选中内容应用或修改自动筛选。
支持按列索引或列名、比较运算符、多值、颜色以及高级筛选运算符进行筛选。
...
| 名称            | 类型                      | 示例             | 描述                                                                                                                                                                                                                               |
| --------------- | ------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| range           | string                    | "A1:D10"         | 应用自动筛选的单元格范围。若省略则使用活动或选中范围。                                                                                                                                                                             |
| field           | number                    | 1                | 用于筛选的字段编号，从最左列起始，编号从 1 开始。                                                                                                                                                                                  |
| fieldName       | string                    | "Price"          | 用于筛选的列名/标题（如 "Name"、"Age"）。函数会自动将其映射到对应的列索引。                                                                                                                                                        |
| criteria1       | string \| array \| object | ">18"            | 筛选条件。可为：运算符字符串（如 ">10"）、多值数组（如 \[1,2,3]）、用于颜色筛选的 [ApiColor](/docs/office-api/usage-api/spreadsheet-api/ApiColor) 对象，或动态筛选常量。                                                           |
| operator        | string                    | "xlFilterValues" | 筛选运算符。可为："xlAnd"、"xlOr"、"xlFilterValues"、"xlTop10Items"、"xlTop10Percent"、"xlBottom10Items"、"xlBottom10Percent"、"xlFilterCellColor"、"xlFilterFontColor"、"xlFilterDynamic"。                                        |
| criteria2       | string                    | "\<2"            | 第二个筛选条件，与 "xlAnd" 或 "xlOr" 运算符配合使用，用于组合筛选。                                                                                                                                                               |
